...
DEFAULT_MAX_SAMPLE_FRAME_GAP = 15
DEFAULT_APPEARANCE_WINDOW_SIZE = 3
DEFAULT_APPEARANCE_CHANGE_THRESHOLD = 0.25
DEFAULT_MIN_CANDIDATE_SEPARATION_SAMPLES = 3


def build_parser():
    parser = argparse.ArgumentParser(
        description=(
            "Split sampled track embeddings at temporal gaps and reviewed "
    ...
    parser.add_argument("--output-prototypes", type=Path, required=True)
    parser.add_argument(
        "--max-sample-frame-gap",
        type=int,
        default=DEFAULT_MAX_SAMPLE_FRAME_GAP,
    )
    parser.add_argument(
        "--appearance-window-size",
        type=int,
        default=DEFAULT_APPEARANCE_WINDOW_SIZE,
    )
    parser.add_argument(
        "--appearance-change-threshold",
        type=float,
        default=DEFAULT_APPEARANCE_CHANGE_THRESHOLD,
    )
    parser.add_argument(
        "--min-candidate-separation-samples",
        type=int,
        default=DEFAULT_MIN_CANDIDATE_SEPARATION_SAMPLES,
    )
    return parser
...
def parse_args(argv=None):
    ...
    if not 0.0 <= args.appearance_change_threshold <= 2.0:
        parser.error("--appearance-change-threshold must be in [0, 2]")
...
def run(args):
    ...
    SEGMENTS_OUTPUT_PATH = args.output_segments
    PROTOTYPES_OUTPUT_PATH = args.output_prototypes
    MAX_SAMPLE_FRAME_GAP = args.max_sample_frame_gap
    APPEARANCE_WINDOW_SIZE = args.appearance_window_size
    APPEARANCE_CHANGE_THRESHOLD = args.appearance_change_threshold
    MIN_CANDIDATE_SEPARATION_SAMPLES = (
        args.min_candidate_separation_samples
    )

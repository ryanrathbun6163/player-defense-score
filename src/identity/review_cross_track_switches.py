...
EMBEDDINGS_PATH = Path(
    "data/outputs/reid/possession_001_osnet_embeddings.npz"
)
...
OUTPUT_DIR = Path(
    "data/outputs/identity/cross_track_switch_review"
)
REPORT_PATH = (
    OUTPUT_DIR
    / "possession_001_cross_track_switch_candidates.json"
)
...
def choose_clean_review_rows(
    ...
    if not rows:
        raise ValueError(
            f"No rows for {player_id} in clean-review "
            f"window {first_frame}-{last_frame}."
        )
...
def build_boundary_record(
    ...
    missing_samples = [
        player_id
        for player_id, samples in (
            list(first_samples.items())
            + list(second_samples.items())
        )
        if not samples
    ]
    ...
    if missing_samples:
        raise ValueError(
            "No local appearance samples for boundary "
            f"{boundary}: {sorted(set(missing_samples))}"
        )
    ...
    return {
        ...
        "interpretation": {
            ...
            "consensus": (
                ...
                "assignments choose the same five pairings."
            ),
            "review_requirement": (
...
def create_assignment_grid(
    ...
    title = residual.title_panel(
        ...
        [
            (
                f"{boundary_record['boundary_id']} | "
                "best five-player lineup assignment"
            ),
            (

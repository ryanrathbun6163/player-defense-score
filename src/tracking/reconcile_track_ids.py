...
DEFAULT_MAX_SEGMENT_OVERLAP_FRAMES = 30
DEFAULT_MAX_SEGMENT_GAP_FRAMES = 30
DEFAULT_MAX_APPEARANCE_DISTANCE = 0.18
DEFAULT_MAX_ENDPOINT_FLOOR_DISTANCE = 120.0
DEFAULT_MIN_ENDPOINT_BOX_IOU = 0.15
DEFAULT_MIN_DUPLICATE_MEDIAN_IOU = 0.45
DEFAULT_MAX_DUPLICATE_MEDIAN_DISTANCE = 45.0
DEFAULT_STRICT_MAX_APPEARANCE_DISTANCE = 0.14
DEFAULT_STRICT_MAX_FRAME_GAP = 10
DEFAULT_STRICT_MAX_FLOOR_DISTANCE = 90.0
DEFAULT_STRICT_MIN_BOX_IOU = 0.20
...
def build_parser():
    ...
    parser.add_argument(
        "--max-segment-overlap-frames", type=int,
        default=DEFAULT_MAX_SEGMENT_OVERLAP_FRAMES,
    )
    parser.add_argument(
        "--max-segment-gap-frames", type=int,
        default=DEFAULT_MAX_SEGMENT_GAP_FRAMES,
    )
    parser.add_argument(
        "--max-appearance-distance", type=float,
        default=DEFAULT_MAX_APPEARANCE_DISTANCE,
    )
    parser.add_argument(
        "--max-endpoint-floor-distance", type=float,
        default=DEFAULT_MAX_ENDPOINT_FLOOR_DISTANCE,
    )
    parser.add_argument(
        "--min-endpoint-box-iou", type=float,
        default=DEFAULT_MIN_ENDPOINT_BOX_IOU,
    )
    parser.add_argument(
        "--min-duplicate-median-iou", type=float,
        default=DEFAULT_MIN_DUPLICATE_MEDIAN_IOU,
    )
    parser.add_argument(
        "--max-duplicate-median-distance", type=float,
        default=DEFAULT_MAX_DUPLICATE_MEDIAN_DISTANCE,
    )
    parser.add_argument(
        "--strict-max-appearance-distance", type=float,
        default=DEFAULT_STRICT_MAX_APPEARANCE_DISTANCE,
    )
    parser.add_argument(
        "--strict-max-frame-gap", type=int,
        default=DEFAULT_STRICT_MAX_FRAME_GAP,
    )
    parser.add_argument(
        "--strict-max-floor-distance", type=float,
        default=DEFAULT_STRICT_MAX_FLOOR_DISTANCE,
    )
    parser.add_argument(
        "--strict-min-box-iou", type=float,
        default=DEFAULT_STRICT_MIN_BOX_IOU,
    )
    return parser
...
def parse_args(argv=None):
    ...
    for name in (
        ...
        if not 0.0 <= getattr(args, name) <= 2.0:
            parser.error(f"--{name.replace('_', '-')} must be in [0, 2]")
...
def run(args):
    ...
    SEGMENT_PROTOTYPES_PATH = args.prototypes
    REVIEW_CONFIG_PATH = args.review_config
    OUTPUT_PATH = args.output
    MAX_SEGMENT_OVERLAP_FRAMES = args.max_segment_overlap_frames
    MAX_SEGMENT_GAP_FRAMES = args.max_segment_gap_frames
    MAX_APPEARANCE_DISTANCE = args.max_appearance_distance
    MAX_ENDPOINT_FLOOR_DISTANCE = args.max_endpoint_floor_distance
    MIN_ENDPOINT_BOX_IOU = args.min_endpoint_box_iou
    MIN_DUPLICATE_MEDIAN_IOU = args.min_duplicate_median_iou
    MAX_DUPLICATE_MEDIAN_DISTANCE = args.max_duplicate_median_distance
    STRICT_MAX_APPEARANCE_DISTANCE = args.strict_max_appearance_distance
    STRICT_MAX_FRAME_GAP = args.strict_max_frame_gap
    STRICT_MAX_FLOOR_DISTANCE = args.strict_max_floor_distance
    STRICT_MIN_BOX_IOU = args.strict_min_box_iou
